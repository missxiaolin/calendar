...
import TimelinePlugin, {
  ...
  ScrollJoiner,
  StickyScroller
} from "@/components/@fullcalendar/timeline";
import ResourceCommonPlugin, {
  ...
  ResourceSplitter,
  buildRowNodes
} from "@/components/@fullcalendar/resource-common";
...
var Row = /** @class */ (function(_super) {
  ...
  function Row(
    ...
    spreadsheetParent.insertBefore(
      (_this.spreadsheetTr = document.createElement("tr")),
      spreadsheetNextSibling
    );
    timeAxisParent.insertBefore(
      (_this.timeAxisTr = document.createElement("tr")),
      timeAxisNextSibling
    );
    return _this;
  }
...
var GroupRow = /** @class */ (function(_super) {
  ...
  function GroupRow() {
    ...
    _this._renderCells = memoizeRendering(
      _this.renderCells,
      _this.unrenderCells
    );
    _this._updateExpanderIcon = memoizeRendering(
      _this.updateExpanderIcon,
      null,
      [_this._renderCells]
    );
  ...
  GroupRow.prototype.renderCells = function(group, spreadsheetColCnt) {
    ...
    this.spreadsheetTr.appendChild(
      createElement(
        ...
        (this.spreadsheetHeightEl = createElement(
          "div",
          null,
          spreadsheetContentEl
        ))
...
var SpreadsheetRow = /** @class */ (function(_super) {
  ...
  function SpreadsheetRow(context, tr) {
    ...
    _this._updateTrResourceId = memoizeRendering(updateTrResourceId, null, [
      _this._renderRow
    ]);
    _this._updateExpanderIcon = memoizeRendering(
      _this.updateExpanderIcon,
  ...
  SpreadsheetRow.prototype.renderRow = function(
    resource,
    rowSpans,
    depth,
    colSpecs
    ...
    for (var i = 0; i < colSpecs.length; i++) {
      ...
      if (typeof colSpec.render === "function") {
        ...
        contentEl =
          colSpec.render(new ResourceApi(calendar, resource), contentEl) ||
          contentEl;
      }
      if (rowSpan > 1) {
  ...
  SpreadsheetRow.prototype.updateExpanderIcon = function(
    hasChildren,
    isExpanded
  ) {
...
var SpreadsheetHeader = /** @class */ (function(_super) {
  ...
  SpreadsheetHeader.prototype.initColResizing = function() {
    var _this = this;
    var ElementDraggingImpl = this.calendar.pluginSystem.hooks
      .elementDraggingImpl;
    if (ElementDraggingImpl) {
      this.resizables = this.resizerEls.map(function(handleEl, colIndex) {
        ...
        dragging.emitter.on("dragmove", function(pev) {
          _this.colWidths[colIndex] = Math.max(
            startWidth + pev.deltaX * (_this.isRtl ? -1 : 1),
            COL_MIN_WIDTH
          );
...
var Spreadsheet = /** @class */ (function(_super) {
  ...
  function Spreadsheet(context, headParentEl, bodyParentEl) {
    ...
    _this.layout = new HeaderBodyLayout(
      headParentEl,
      bodyParentEl,
      "clipped-scroll"
    );
    ...
    _this.header = new SpreadsheetHeader(
      context,
    ...
    bodyEnhancedScroller.canvas.contentEl.appendChild(
...
var ResourceTimelineView = /** @class */ (function(_super) {
  ...
  function ResourceTimelineView(
    ...
    for (
      var _i = 0, allColSpecs_1 = allColSpecs;
      _i < allColSpecs_1.length;
      _i++
    ) {
    ...
    var plainOrderSpecs = [];
    for (
      var _a = 0, allOrderSpecs_1 = allOrderSpecs;
      _a < allOrderSpecs_1.length;
      _a++
    ) {
      var orderSpec = allOrderSpecs_1[_a];
      var isGroup = false;
      for (
        var _b = 0, groupSpecs_1 = groupSpecs;
        _b < groupSpecs_1.length;
        _b++
      ) {
    ...
    _this.resourceAreaHeadEl = _this.el.querySelector(
    ...
    _this.spreadsheet = new Spreadsheet(
      _this.context,
      _this.resourceAreaHeadEl,
      _this.el.querySelector("tbody .fc-resource-area")
    );
    ...
    var timeAxisRowContainer = createElement(
      "div",
      { className: "fc-rows" },
      "<table><tbody /></table>"
    );
    _this.timeAxis.layout.bodyScroller.enhancedScroll.canvas.contentEl.appendChild(
      timeAxisRowContainer
    );
    ...
    // after scrolljoiner
    _this.spreadsheetBodyStickyScroller = new StickyScroller(
      _this.spreadsheet.layout.bodyScroller.enhancedScroll,
      _this.isRtl,
      true // isVertical
    );
  ...
  ResourceTimelineView.prototype.render = function(props) {
    ...
    var hasResourceBusinessHours = this.hasResourceBusinessHours(
      props.resourceStore
    );
    ...
    var newRowNodes = this.buildRowNodes(
      props.resourceStore,
      this.groupSpecs,
      this.orderSpecs,
      this.isVGrouping,
  ...
  ResourceTimelineView.prototype.addRow = function(index, rowNode) {
    ...
    var newComponent = this.buildChildComponent(
      rowNode,
  ...
  ResourceTimelineView.prototype.removeRows = function(
    startIndex,
    len,
    oldRowNodes
  ) {
  ...
  ResourceTimelineView.prototype.buildChildComponent = function(
    node,
    spreadsheetTbody,
    spreadsheetNext,
    timeAxisTbody,
  ...
  ResourceTimelineView.prototype.renderRows = function(
    dateProfile,
    ...
    for (var i = 0; i < rowNodes.length; i++) {
      ...
      if (rowNode.group) {
        ...
        rowComponent.receiveProps(
          __assign({}, splitProps[resource.id], {
  ...
  ResourceTimelineView.prototype.updateSize = function(
    isResize,
    viewHeight,
    isAuto
  ) {
  ...
  ResourceTimelineView.prototype.updateRowSizes = function(isResize) {
    ...
    for (
      var _b = 0, dirtyRowComponents_1 = dirtyRowComponents;
      _b < dirtyRowComponents_1.length;
      _b++
    ) {
  ...
  ResourceTimelineView.prototype.destroy = function() {
    ...
    for (
      var _b = 0, _c = this.resourceAreaWidthDraggings;
      _b < _c.length;
      _b++
    ) {
  ...
  ResourceTimelineView.prototype.queryResourceScroll = function() {
    var _a = this,
      ...
      rowNodes = _a.rowNodes;
    var scroll = {};
    var scrollerTop = this.timeAxis.layout.bodyScroller.el.getBoundingClientRect()
      .top; // fixed position
  ...
  ResourceTimelineView.prototype.applyResourceScroll = function(scroll) {
    ...
    if (rowId) {
      ...
      if (rowComponent) {
        var el = rowComponent.timeAxisTr;
        if (el) {
          var innerTop = this.timeAxis.layout.bodyScroller.enhancedScroll.canvas.el.getBoundingClientRect()
            .top;
          var rowRect = el.getBoundingClientRect();
          var scrollTop =
            (scroll.forcedRowId
              ? rowRect.top // just use top edge
              : rowRect.bottom - scroll.bottom) - innerTop; // pixels from bottom edge
  ...
  ResourceTimelineView.prototype.queryHit = function(
    positionLeft,
    positionTop
  ) {
  ...
  ResourceTimelineView.prototype.initResourceAreaWidthDragging = function() {
    var _this = this;
    var resourceAreaDividerEls = Array.prototype.slice.call(
      this.el.querySelectorAll(".fc-col-resizer")
    );
    var ElementDraggingImpl = this.calendar.pluginSystem.hooks
      .elementDraggingImpl;
    if (ElementDraggingImpl) {
      this.resourceAreaWidthDraggings = resourceAreaDividerEls.map(function(
        el
      ) {
        ...
        dragging.emitter.on("dragstart", function() {
          ...
          if (typeof dragStartWidth !== "number") {
            dragStartWidth = _this.resourceAreaHeadEl.getBoundingClientRect()
              .width;
          }
