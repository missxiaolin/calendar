...
function computeResourceEditable(eventDef, calendar) {
  ...
  if (resourceEditable == null) {
    var source =
      eventDef.sourceId && calendar.state.eventSources[eventDef.sourceId];
    if (source) {
...
var ResourceDataAdder = /** @class */ (function() {
  ...
  ResourceDataAdder.prototype.transform = function(
    viewProps,
    viewSpec,
    calendarProps,
    view
...
function filterResources(
  resourceStore,
  doFilterResourcesWithEvents,
  eventStore,
  activeRange
) {
  if (doFilterResourcesWithEvents) {
    var instancesInRange = filterEventInstancesInRange(
...
function computeHasEvents(eventInstances, eventDefs) {
  ...
  for (var instanceId in eventInstances) {
    ...
    for (
      var _i = 0, _a = eventDefs[instance.defId].resourceIds;
      _i < _a.length;
      _i++
    ) {
...
var ResourceEventConfigAdder = /** @class */ (function() {
  function ResourceEventConfigAdder() {
    this.buildResourceEventUis = memoizeOutput(
      buildResourceEventUis,
      isPropsEqual
    );
    this.injectResourceEventUis = memoize(injectResourceEventUis);
  }
  ResourceEventConfigAdder.prototype.transform = function(
    viewProps,
    viewSpec,
    calendarProps
  ) {
...
function parseResourceSource(input) {
  for (var i = defs.length - 1; i >= 0; i--) {
    ...
    if (meta) {
      var res = parseResourceSourceProps(
...
function parseResource(input, parentId, store, calendar) {
  ...
  props.businessHours = props.businessHours
    ? parseBusinessHours(props.businessHours, calendar)
    : null;
  props.ui = ui;
  ...
  switch (action.type) {
    ...
    case "SET_RESOURCE_ENTITY_EXPANDED":
      return __assign(
        {},
        expansions,
        ((_a = {}), (_a[action.id] = action.isExpanded), _a)
      );
    default:
      return expansions;
  }
...
function resourcesReducers(state, action, calendar) {
  var resourceSource = reduceResourceSource(
    state.resourceSource,
    action,
    state.dateProfile,
    calendar
  );
  var resourceStore = reduceResourceStore(
    state.resourceStore,
    action,
    resourceSource,
    calendar
  );
  var resourceEntityExpansions = reduceResourceEntityExpansions(
    state.resourceEntityExpansions,
    action
  );
...
function parseEventDef(def, props, leftovers) {
  var resourceRelatedProps = refineProps(
    props,
...
function transformDateSelectionJoin(hit0, hit1) {
  ...
  if (resourceId0 && resourceId1) {
    if (
      hit0.component.allowAcrossResources === false &&
...
function isPropsValidWithResources(props, calendar) {
  ...
  var sets = splitter.splitProps(
    __assign({}, props, { resourceStore: calendar.state.resourceStore })
  );
...
function transformEventResizeJoin(hit0, hit1) {
  ...
  if (
    component.allowAcrossResources === false &&
...
var ResourceDayHeader = /** @class */ (function(_super) {
  ...
  function ResourceDayHeader(context, parentEl) {
    ...
    _this.resourceTextFunc = buildResourceTextFunc(
      _this.opt("resourceText"),
      _this.calendar
    );
  ...
  ResourceDayHeader.prototype.renderDayAndResourceRows = function(
    dates,
    resources
  ) {
    var dateHtmls = [];
  ...
  ResourceDayHeader.prototype.renderResourceAndDayRows = function(
    resources,
    dates
  ) {
    var resourceHtmls = [];
  ...
  ResourceDayHeader.prototype.renderResourceCell = function(
    resource,
    colspan,
    date
  ) {
  ...
  ResourceDayHeader.prototype.renderDateCell = function(
    date,
    colspan,
    resource
  ) {
...
var ResourceDayTable = /** @class */ (function(_super) {
  ...
  /*
    ...
    */
  ResourceDayTable.prototype.computeColRanges = function(
    dateStartI,
    dateEndI,
    resourceI
  ) {
...
var DayResourceTable = /** @class */ (function(_super) {
  ...
  /*
    ...
    */
  DayResourceTable.prototype.computeColRanges = function(
    dateStartI,
    dateEndI,
    resourceI
  ) {
  ...
  VResourceJoiner.prototype.joinProps = function(propSets, resourceDayTable) {
    ...
    return {
      dateSelectionSegs: this.joinDateSelection.apply(
        this,
        [resourceDayTable].concat(dateSelectionSets)
      ),
      businessHourSegs: this.joinBusinessHours.apply(
        this,
        [resourceDayTable].concat(businessHoursSets)
      ),
      fgEventSegs: this.joinFgEvents.apply(
        this,
        [resourceDayTable].concat(fgEventSets)
      ),
      bgEventSegs: this.joinBgEvents.apply(
        this,
        [resourceDayTable].concat(bgEventSets)
      ),
      eventDrag: this.joinEventDrags.apply(
        this,
        [resourceDayTable].concat(eventDrags)
      ),
      eventResize: this.joinEventResizes.apply(
        this,
        [resourceDayTable].concat(eventResizes)
      ),
  ...
  VResourceJoiner.prototype.joinInteractions = function(resourceDayTable) {
    ...
    for (var i = 0; i < resourceCnt; i++) {
      ...
      if (interactions[resourceCnt]) {
        // one beyond. the all-resource
        for (
          var _c = 0, _d = interactions[resourceCnt].segs;
          _c < _d.length;
          _c++
        ) {
          var seg = _d[_c];
...
function flattenResources(resourceStore, orderSpecs) {
  return buildRowNodes(resourceStore, [], orderSpecs, false, {}, true).map(
    function(node) {
...
function buildRowNodes(
  ...
  var complexNodes = buildHierarchy(
    resourceStore,
    isVGrouping ? -1 : 1,
    groupSpecs,
    orderSpecs
  ...
  for (var i = 0; i < complexNodes.length; i++) {
    ...
    if (group) {
      if (isVGrouping) {
        ...
        var isExpanded =
          expansions[id] != null ? expansions[id] : expansionDefault;
...
function ensureGroupNodes(resourceNode, nodes, groupSpec) {
  ...
  if (groupSpec.order) {
    for (newGroupIndex = 0; newGroupIndex < nodes.length; newGroupIndex++) {
      ...
      if (node.group) {
        var cmp =
          flexibleCompare(groupValue, node.group.value) * groupSpec.order;
        if (cmp === 0) {
          groupNode = node;
          break;
        } else if (cmp < 0) {
