...
import {
  ...
  View,
  createPlugin
} from "@/components/@fullcalendar/core";
  ...
  EnhancedScroller.prototype.bindPreventTouchScroll = function() {
    if (!this.preventTouchScrollHandler) {
      this.el.addEventListener(
        "touchmove",
        (this.preventTouchScrollHandler = preventDefault)
      );
    }
  };
        ...
        if (scroller === _this.masterScroller) {
          for (var _i = 0, _a = _this.scrollers; _i < _a.length; _i++) {
            ...
            if (otherScroller !== scroller) {
              switch (_this.axis) {
                case "horizontal":
                  otherScroller.enhancedScroll.el.scrollLeft =
                    enhancedScroll.el.scrollLeft;
                  break;
                case "vertical":
                  otherScroller.enhancedScroll.setScrollTop(
                    enhancedScroll.getScrollTop()
                  );
                  break;
              }
            }
...
var HeaderBodyLayout = /** @class */ (function() {
  /*
    ...
    */
  function HeaderBodyLayout(
    headerContainerEl,
    bodyContainerEl,
    verticalScroll
  ) {
    this.headerScroller = new ClippedScroller(
      "clipped-scroll",
      "hidden",
      headerContainerEl
    );
...
var TimelineHeader = /** @class */ (function(_super) {
  ...
  function TimelineHeader(context, parentEl) {
    ...
    parentEl.appendChild(
      (_this.tableEl = createElement("table", {
  ...
  TimelineHeader.prototype.renderDates = function(tDateProfile) {
    ...
    var isChrono =
      asRoughMs(tDateProfile.labelInterval) >
      asRoughMs(tDateProfile.slotDuration);
    ...
    for (var _i = 0, cellRows_1 = cellRows; _i < cellRows_1.length; _i++) {
      ...
      for (var _b = 0, rowCells_1 = rowCells; _b < rowCells_1.length; _b++) {
        ...
        html +=
          '<th class="' +
          headerCellClassNames.join(" ") +
          '"' +
          ' data-date="' +
          dateEnv.formatIso(cell.date, {
    ...
    this.innerEls = findElements(
      this.tableEl.querySelector("tr:last-child"), // compound selector won't work because of query-root problem
      "th .fc-cell-text"
    );
    findElements(
...
var TimelineSlats = /** @class */ (function(_super) {
  ...
  function TimelineSlats(context, parentEl) {
    var _this = _super.call(this, context) || this;
    parentEl.appendChild(
      (_this.el = createElement("div", { className: "fc-slats" }))
    );
    return _this;
  }
  ...
  TimelineSlats.prototype.renderDates = function(tDateProfile) {
    ...
    var html =
      '<table class="' + theme.getClass("tableGrid") + '">' + "<colgroup>";
...
function buildTimelineDateProfile(dateProfile, view) {
  ...
  if (rawSnapDuration) {
    ...
    snapsPerSlot = wholeDivideDurations(
  ...
  // more...
  var timeWindowMs =
    asRoughMs(dateProfile.maxTime) - asRoughMs(dateProfile.minTime);
  // TODO: why not use normalizeRange!?
  var normalizedStart = normalizeDate(
    dateProfile.renderRange.start,
    tDateProfile,
    dateEnv
  );
  var normalizedEnd = normalizeDate(
    dateProfile.renderRange.end,
    tDateProfile,
    dateEnv
  );
...
function normalizeRange(range, tDateProfile, dateEnv) {
  if (!tDateProfile.isTimeScale) {
    ...
    if (tDateProfile.largeUnit) {
      ...
      if (
        range.end.valueOf() !== dayRange.end.valueOf() ||
...
function validateLabelAndSlot(tDateProfile, dateProfile, dateEnv) {
  ...
  if (tDateProfile.labelInterval) {
    var labelCnt = dateEnv.countDurationsBetween(
      currentRange.start,
      currentRange.end,
      tDateProfile.labelInterval
    );
  ...
  if (tDateProfile.slotDuration) {
    var slotCnt = dateEnv.countDurationsBetween(
      currentRange.start,
      currentRange.end,
      tDateProfile.slotDuration
    );
  ...
  if (tDateProfile.labelInterval && tDateProfile.slotDuration) {
    var slotsPerLabel = wholeDivideDurations(
      tDateProfile.labelInterval,
      tDateProfile.slotDuration
    );
...
function ensureLabelInterval(tDateProfile, dateProfile, dateEnv) {
  ...
  if (!labelInterval) {
    ...
    if (tDateProfile.slotDuration) {
      for (
        var _i = 0, STOCK_SUB_DURATIONS_1 = STOCK_SUB_DURATIONS;
        _i < STOCK_SUB_DURATIONS_1.length;
        _i++
      ) {
        ...
        if (
          slotsPerLabel !== null &&
          slotsPerLabel <= MAX_AUTO_SLOTS_PER_LABEL
        ) {
      ...
      for (
        var _a = 0, STOCK_SUB_DURATIONS_2 = STOCK_SUB_DURATIONS;
        _a < STOCK_SUB_DURATIONS_2.length;
        _a++
      ) {
...
function ensureSlotDuration(tDateProfile, dateProfile, dateEnv) {
  ...
  if (!slotDuration) {
    ...
    for (
      var _i = 0, STOCK_SUB_DURATIONS_3 = STOCK_SUB_DURATIONS;
      _i < STOCK_SUB_DURATIONS_3.length;
      _i++
    ) {
...
function buildCellObject(date, text, rowUnit, view) {
  var spanHtml = buildGotoAnchorHtml(
    view,
    {
      date: date,
      type: rowUnit,
...
var StickyScroller = /** @class */ (function() {
  function StickyScroller(scroller, isRtl, isVertical) {
    ...
    this.updateSize = function() {
      var els = Array.prototype.slice.call(
        _this.scroller.canvas.el.querySelectorAll("." + STICKY_CLASSNAME)
      );
      var elGeoms = _this.queryElGeoms(els);
      var viewportWidth = _this.scroller.el.clientWidth;
      if (_this.usingRelative) {
        var elDestinations = _this.computeElDestinations(
          elGeoms,
          viewportWidth
        ); // read before prepPositioning
  ...
  StickyScroller.prototype.computeElDestinations = function(
    elGeoms,
    viewportWidth
  ) {
...
var TimeAxis = /** @class */ (function(_super) {
  ...
  function TimeAxis(context, headerContainerEl, bodyContainerEl) {
    var _this = _super.call(this, context) || this;
    var layout = (_this.layout = new HeaderBodyLayout(
      headerContainerEl,
      bodyContainerEl,
      "auto"
    ));
    ...
    _this.headStickyScroller = new StickyScroller(
      headerEnhancedScroller,
      _this.isRtl,
      false
    ); // isVertical=false
    _this.bodyStickyScroller = new StickyScroller(
      bodyEnhancedScroller,
      _this.isRtl,
      false
    ); // isVertical=false
    _this.header = new TimelineHeader(
      context,
      headerEnhancedScroller.canvas.contentEl
    );
    _this.slats = new TimelineSlats(context, bodyEnhancedScroller.canvas.bgEl);
    _this.nowIndicator = new TimelineNowIndicator(
  ...
  TimeAxis.prototype.render = function(props) {
    var tDateProfile = (this.tDateProfile = buildTimelineDateProfile(
      props.dateProfile,
      this.view
    )); // TODO: cache
  ...
  TimeAxis.prototype.computeDefaultSlotWidth = function(tDateProfile) {
    ...
    this.header.innerEls.forEach(function(innerEl, i) {
      maxInnerWidth = Math.max(
        maxInnerWidth,
        innerEl.getBoundingClientRect().width
      );
    });
  ...
  TimeAxis.prototype.applySlotWidth = function(slotWidth) {
    ...
    if (slotWidth !== "") {
      ...
      if (availableWidth > containerWidth) {
        ...
        containerWidth = "";
        nonLastSlotWidth = Math.floor(
          availableWidth / tDateProfile.slotDates.length
        );
      }
    }
  ...
  TimeAxis.prototype.computeDateSnapCoverage = function(date) {
    var _a = this,
      dateEnv = _a.dateEnv,
      tDateProfile = _a.tDateProfile;
    var snapDiff = dateEnv.countDurationsBetween(
      tDateProfile.normalizedRange.start,
      date,
...
var TimelineLaneEventRenderer = /** @class */ (function(_super) {
  ...
  TimelineLaneEventRenderer.prototype.renderSegHtml = function(
    seg,
    mirrorInfo
  ) {
    ...
    var isResizableFromStart =
      seg.isStart && view.computeEventStartResizable(eventDef, eventUi);
    ...
    return (
      ...
      classes.join(" ") +
      '" style="' +
      cssToStr(this.getSkinCss(eventUi)) +
      '"' +
...
var TimelineLane = /** @class */ (function(_super) {
  ...
  function TimelineLane(context, fgContainerEl, bgContainerEl, timeAxis) {
    var _this =
      _super.call(this, context, bgContainerEl) || this; // should el be bgContainerEl???
    ...
    var fillRenderer = (_this.fillRenderer = new TimelineLaneFillRenderer(
      context,
      bgContainerEl,
      timeAxis
    ));
    var eventRenderer = (_this.eventRenderer = new TimelineLaneEventRenderer(
      context,
      fgContainerEl,
      timeAxis
    ));
    _this.mirrorRenderer = new TimelineLaneEventRenderer(
      context,
      fgContainerEl,
      timeAxis
    );
  ...
  TimelineLane.prototype.render = function(props) {
    var slicedProps = this.slicer.sliceProps(
      props,
      props.dateProfile,
...
var TimelineLaneSlicer = /** @class */ (function(_super) {
  ...
  TimelineLaneSlicer.prototype.sliceRange = function(origRange, timeAxis) {
    ...
    if (
      timeAxis.computeDateSnapCoverage(normalRange.start) <
      timeAxis.computeDateSnapCoverage(normalRange.end)
    ) {
      // intersect the footprint's range with the grid's range
      var slicedRange = intersectRanges(
        normalRange,
...
var TimelineView = /** @class */ (function(_super) {
  ...
  function TimelineView(context, viewSpec, dateProfileGenerator, parentEl) {
    var _this =
      _super.call(this, context, viewSpec, dateProfileGenerator, parentEl) ||
      this;
    ...
    _this.timeAxis = new TimeAxis(
      _this.context,
      _this.el.querySelector("thead .fc-time-area"),
      _this.el.querySelector("tbody .fc-time-area")
    );
    _this.lane = new TimelineLane(
      _this.context,
      _this.timeAxis.layout.bodyScroller.enhancedScroll.canvas.contentEl,
      _this.timeAxis.layout.bodyScroller.enhancedScroll.canvas.bgEl,
      _this.timeAxis
    );
  ...
  TimelineView.prototype.render = function(props) {
    ...
    this.lane.receiveProps(
      __assign({}, props, { nextDayThreshold: this.nextDayThreshold })
    );
  };
  ...
  TimelineView.prototype.queryHit = function(
    positionLeft,
    positionTop,
    elWidth,
    elHeight
